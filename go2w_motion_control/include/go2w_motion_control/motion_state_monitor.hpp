...
struct MotionEvidence {
  ...
  std::array<double, 4> q_net{};
  std::array<double, 4> dq_p95_abs{};
  // Median of the four signed wheel position deltas.  On this Go2-W all
  // wheel q values increase for forward motion and decrease for reverse.
  // The median rejects one slipping/noisy wheel while preserving direction.
  double signed_wheel_travel_rad{0.0};
};
