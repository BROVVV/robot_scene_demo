...
MotionActionServer::Parameters MotionActionServer::LoadParameters() {
  ...
  if (const char *launch_dry_run = std::getenv("GO2W_LAUNCH_DRY_RUN")) {
    const std::string value(launch_dry_run);
    if (value == "true" || value == "1") {
      ...
      throw std::invalid_argument(
          "GO2W_LAUNCH_DRY_RUN must be true/false or 1/0");
    }
    // Keep introspection consistent with the safety mode actually in force.
    set_parameter(rclcpp::Parameter("dry_run", p.dry_run));
  }
  ...
  p.post_timed_zero_velocity_hold_sec = declare_parameter<double>(
      "post_timed_zero_velocity_hold_sec", 0.8);
  ...
  p.timed_min_distance_ratio =
      declare_parameter<double>("timed_min_distance_ratio", 0.30);
  p.timed_min_distance_m =
      declare_parameter<double>("timed_min_distance_m", 0.03);
  if (!std::isfinite(p.turn_longitudinal_compensation_vx) ||
      ...
      !std::isfinite(p.timed_min_distance_ratio) ||
      p.timed_min_distance_ratio <= 0.0 ||
      p.timed_min_distance_ratio > 1.0 ||
      !std::isfinite(p.timed_min_distance_m) ||
      p.timed_min_distance_m < 0.0 ||
      p.timed_min_distance_m > 0.2 ||
      !std::isfinite(p.post_timed_zero_velocity_hold_sec) ||
      p.post_timed_zero_velocity_hold_sec < 0.0 ||
      p.post_timed_zero_velocity_hold_sec > 2.0) {
    throw std::invalid_argument(
        "invalid turn compensation, rollback control, wheel radius, or timed "
        "distance verification parameters");
  }
...
void MotionActionServer::HandleEmergencyStop(
  ...
  if (stopped) {
    response->message =
        status == 0
            ? "STOP acknowledged; robot stationary"
            : "STOP dispatched; acknowledgement timed out, but fresh wheel "
              "state confirms robot stationary";
  } else {
    response->message =
...
void MotionActionServer::RunTimedVelocity(
  ...
  // StopMove releases the high-level wheel velocity controller.  On the real
  // Go2-W that allowed reverse momentum to coast anywhere from a few to ten
  // centimetres.  Keep ai-w actively commanding zero before the final
  // redundant StopMove sequence so the timed primitive has a bounded end.
  if (parameters_.post_timed_zero_velocity_hold_sec > 0.0) {
    logger->LogSafety("post_timed_zero_velocity_hold", "started");
    const auto hold_started = std::chrono::steady_clock::now();
    auto next_publish = hold_started;
    const auto publish_period =
        std::chrono::duration_cast<std::chrono::steady_clock::duration>(
            std::chrono::duration<double>(
                1.0 / parameters_.turn_publish_max_hz));
    while (std::chrono::duration<double>(std::chrono::steady_clock::now() -
                                         hold_started)
               .count() < parameters_.post_timed_zero_velocity_hold_sec) {
      CheckRuntimeSafety(goal_handle, logger);
      const auto now = std::chrono::steady_clock::now();
      if (now >= next_publish) {
        const auto response = sport_client_->SendMove(
        ...
        if (!response.response_received || response.status_code != 0) {
          throw MotionFailure{
              MotionCommand::Result::ERROR_MOVE_REJECTED,
              "post-timed zero-velocity hold failed", false};
        }
        next_publish = now + publish_period;
      }
      execution.elapsed_sec = std::chrono::duration<float>(
                                  now - execution.started)
                                  .count();
      PublishFeedback(goal_handle, execution, 0.0, 0.0, 0.0, logger);
      std::this_thread::sleep_for(50ms);
    }
    execution.post_timed_zero_velocity_hold_completed = true;
    logger->LogSafety("post_timed_zero_velocity_hold", "completed");
  }
}
...
void MotionActionServer::Execute(
  ...
  const double signed_wheel_distance_m =
      evidence.signed_wheel_travel_rad * parameters_.wheel_radius_m;
  const bool timed_linear_goal =
      goal->mode == MotionCommand::Goal::MODE_TIMED_VELOCITY &&
      std::abs(goal->vx) > 1e-4;
  const double expected_timed_distance_m =
      timed_linear_goal
          ? std::abs(static_cast<double>(goal->vx) *
                     static_cast<double>(goal->duration_sec))
          : 0.0;
  const double required_timed_distance_m =
      timed_linear_goal
          ? std::min(
                expected_timed_distance_m,
                std::max(parameters_.timed_min_distance_m,
                         expected_timed_distance_m *
                             parameters_.timed_min_distance_ratio))
          : 0.0;
  const double projected_timed_distance_m =
      timed_linear_goal
          ? std::copysign(1.0, static_cast<double>(goal->vx)) *
                signed_wheel_distance_m
          : 0.0;
  execution.elapsed_sec = std::chrono::duration<float>(
  ...
  if (!stopped && motion_completed) {
    ...
    motion_completed = false;
  } else if (motion_completed && !parameters_.dry_run && timed_linear_goal &&
             projected_timed_distance_m + 1e-6 <
                 required_timed_distance_m) {
    std::ostringstream message;
    message << std::fixed << std::setprecision(3)
            << "RPC succeeded but signed wheel distance "
            << signed_wheel_distance_m << "m did not confirm commanded "
            << (goal->vx > 0.0F ? "forward" : "reverse")
            << " motion (required " << required_timed_distance_m
            << "m in the commanded direction)";
    failure = {MotionCommand::Result::ERROR_MOVE_REJECTED, message.str(),
               false};
    motion_completed = false;
  } else if (motion_completed && !parameters_.dry_run &&
             !timed_linear_goal && !evidence.strong) {
  ...
  result->estimated_distance_m =
      timed_linear_goal ? signed_wheel_distance_m
                        : state_monitor_->EstimatedDistance();
  ...
  logger->LogResult({{"success", result->success},
                     ...
                     {"post_timed_zero_velocity_hold_sec",
                      parameters_.post_timed_zero_velocity_hold_sec},
                     {"post_timed_zero_velocity_hold_completed",
                      execution.post_timed_zero_velocity_hold_completed},
                     ...
                     {"timed_min_distance_ratio",
                      parameters_.timed_min_distance_ratio},
                     {"timed_min_distance_m",
                      parameters_.timed_min_distance_m},
                     {"expected_timed_distance_m",
                      expected_timed_distance_m},
                     {"required_timed_distance_m",
                      required_timed_distance_m},
                     {"projected_timed_distance_m",
                      projected_timed_distance_m},
                     {"signed_wheel_distance_m",
                      signed_wheel_distance_m},
                     ...
                     {"wheel_q_net", evidence.q_net},
                     {"signed_wheel_travel_rad",
                      evidence.signed_wheel_travel_rad},
