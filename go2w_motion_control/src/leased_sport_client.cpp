...
constexpr int32_t kClientApiTimeout = 3104;
...
RequestResult LeasedSportClient::SendRequest(
    ...
    std::chrono::milliseconds timeout, bool force_dispatch) {
  ...
  if (!force_dispatch && (!LeaseAvailable() || result.lease_id == 0)) {
...
RequestResult LeasedSportClient::SendStopMove(
    std::chrono::milliseconds timeout, bool force_dispatch) {
  return SendRequest(kStopApi, "{}", timeout, force_dispatch);
}
...
bool LeasedSportClient::StopRepeatedly(
  ...
  int dispatched_timeouts = 0;
  int32_t status = -9999;
  for (int attempt = 0; attempt < count; ++attempt) {
    // STOP is a best-effort safety command and must reach the lease-owning
    // executor even when the ROS status bridge is stale. The executor performs
    // the authoritative GetLeaseId check before calling the SDK.
    const auto result = SendStopMove(response_timeout, true);
    status = result.status_code;
    if (result.response_received && result.status_code == 0) {
      ++successful_responses;
    } else if (result.response_received &&
               result.status_code == kClientApiTimeout) {
      // The lease-owning SDK executor did dispatch StopMove but Unitree did
      // not return its RPC acknowledgement before the deadline.  The caller
      // still requires fresh stationary wheel evidence before accepting STOP.
      ++dispatched_timeouts;
    }
    if (attempt + 1 < count) std::this_thread::sleep_for(interval);
  }
  // Repetition increases delivery probability. Requiring every redundant
  // acknowledgement made one late reply override two confirmed STOPs and
  // fresh stationary wheel evidence.
  if (last_status != nullptr) {
    *last_status = successful_responses > 0 ? 0 : status;
  }
  return successful_responses > 0 || dispatched_timeouts > 0;
}
