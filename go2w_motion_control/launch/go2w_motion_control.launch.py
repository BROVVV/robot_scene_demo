...
def generate_launch_description() -> LaunchDescription:
    ...
    sdk_rpc_timeout_sec = LaunchConfiguration("sdk_rpc_timeout_sec")
    ...
    action_server = Node(
        ...
        respawn=True,
        respawn_delay=1.0,
        # Exact-node YAML entries outrank launch_ros' generated /** override
        # file on this ROS 2 deployment. Keep an independent, authoritative
        # dry-run signal so a custom config file cannot silently enable motion.
        additional_env={"GO2W_LAUNCH_DRY_RUN": dry_run},
        parameters=[
            config_file,
            {
                "robot_ip": robot_ip,
                # Foxy otherwise serializes LaunchConfiguration booleans as
                # strings in this mixed parameter-file/override list. Explicit
                # typing guarantees the launch override wins over YAML false.
                "require_arm": ParameterValue(require_arm, value_type=bool),
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("sdk_rpc_timeout_sec", default_value="1.0"),
            DeclareLaunchArgument(
